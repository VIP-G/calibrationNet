...
from models.DatasetLidarCamera_raw import DatasetKitti360
from models.quaternion_distances import quaternion_distance
from models.utils import (mat2xyzrpy, overlay_imgs,overlay_reflect_imgs,
...
weights = [
    # "checkpoints/checkpoint_r20.00_t1.50_e49_val5.912.pth",
    # "checkpoints/checkpoint_r10.00_t1.00_e116_val2.193.pth",
    # "checkpoints/checkpoint_r5.00_t0.50_e47_val1.158.pth",
    # "checkpoints/checkpoint_r2.00_t0.20_e46_val0.206.pth",
    # "checkpoints/r1_fwq_e50_3e-4/checkpoint_r1.00_t0.10_e48_val0.234.pth"

    # "checkpoints-kitti360-10e/r20_t1_5/val_seq_03/models/checkpoint_r20.00_t1.50_e9_val36.552.pth",
    # "checkpoints-kitti360-10e/r10_t1/val_seq_03/models/checkpoint_r10.00_t1.00_e9_val17.563.pth",
    # "checkpoints-kitti360-10e/r5_t0_5/models/checkpoint_r5.00_t0.50_e9_val10.623.pth",
    # "checkpoints-kitti360-10e/r2_t0_2/val_seq_03/models/checkpoint_r2.00_t0.20_e8_val5.387.pth",
    # "checkpoints-kitti360-10e/r1_t0_1/val_seq_03/models/checkpoint_r1.00_t0.10_e10_val2.155.pth"

    # "checkpoints-kitti360-10e/r20_t1_5/val_seq_03/models/checkpoint_r20.00_t1.50_e1_val61.725.pth",
    # "checkpoints-kitti360-10e/r10_t1/val_seq_03/models/checkpoint_r10.00_t1.00_e1_val29.648.pth",
    # "checkpoints-kitti360-10e/r5_t0_5/models/checkpoint_r5.00_t0.50_e1_val13.939.pth",
    # "checkpoints-kitti360-10e/r2_t0_2/val_seq_03/models/checkpoint_r2.00_t0.20_e1_val7.680.pth",
    # "checkpoints-kitti360-10e/r1_t0_1/val_seq_03/models/checkpoint_r1.00_t0.10_e1_val2.754.pth"


    # 'checkpointskitti/r20_t1-5/val_seq_03/models/checkpoint_r20.00_t1.50_e3_val0.628.pth'
    # 'checkpointsZGH/test0721log/val_seq_05/models/checkpoint_r20.00_t1.50_e3_val0.144.pth'
    'checkpointsZGH/test0721log/val_seq_05/models/checkpoint_r20.00_t1.50_e12_val0.025.pth'
...
def val(args):
    ...
    if args.save_log:
        ...
        log_file = os.path.join(log_file_path, f'log_seq{args.test_sequence}.csv')
    ...
    for batch_idx, sample in enumerate(tqdm(TestImgLoader)):
        ...
        for idx in range(len(sample['rgb'])):
            ...
            # depth_gt, uv_gt, pc_gt_valid = lidar_project_depth(
            #     pc_lidar, sample['calib'][idx], real_shape)  # image_shape
            ...
            # if args.max_depth < 80.:
            #     pc_rotated = pc_rotated[:, pc_rotated[
            #         0, :] < args.max_depth].clone()

            # depth_img, uv_input, pc_input_valid = lidar_project_depth(
            #     pc_rotated, sample['calib'][idx], real_shape)  # image_shape
        ...
        with torch.no_grad():
            for iteration in range(start, len(weights)):
                ...
                if args.use_reflectance:
                    ...
                    depth_img_pred = torch.cat([depth_img_pred, reflect_img_pred], dim=0)
                else:
                    depth_img_pred = depth_img_pred

                # depth_img_pred, uv_pred, pc_pred_valid = lidar_project_depth(
                #     rotated_point_cloud, sample['calib'][0],
                #     real_shape_input[0])  # image_shape
                # depth_img_pred /= args.max_depth
    ...
    if args.save_name is not None:
        ...
        torch.save(
            ...
            f'output360/results_for_paper/{args.save_name}_errors_t')
        
        torch.save(
            torch.stack(errors_r).cpu().numpy(),
            f'output360/results_for_paper/{args.save_name}_errors_r')
        
        torch.save(
            torch.stack(errors_t2).cpu().numpy(),
            f'output360/results_for_paper/{args.save_name}_errors_t2')
        
        torch.save(
            torch.stack(errors_rpy).cpu().numpy(),
            f'output360/results_for_paper/{args.save_name}_errors_rpy')
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--data_folder',
        default= '/data_home/home/zhangguanghui/kitti360',#'/media/bolo/ubuntu/LCCNet_Dataset/odometry/dataset',
        help='')
    parser.add_argument('--use_prev_output',
                        action='store_true',
                        default=False,
                        help='')
    ...
    parser.add_argument('--show', action='store_true', default=False,
                        help='')  #False
    parser.add_argument('--use_reflectance',
                        action='store_true',
                        default=True,
                        help='')
    ...
    parser.add_argument('--weight',
                        default=None,
                        help=' List of weights path for iterative refinement')
    parser.add_argument('--save_name', default='kitti360725', help=' ')
    # Set to True only if you use two network, the first for rotation and the second for translation
    parser.add_argument('--rot_transl_separated',
                        action='store_true',
                        default=False,
                        help='')
    parser.add_argument('--random_initial_pose',
                        action='store_true',
                        default=False,
                        help='')
    parser.add_argument(
    ...
    parser.add_argument('--iterative_method', default='single',
                        help='')  #'multi_range', help="")#'single'
    parser.add_argument('--test_sequence', default="05", help='')
    parser.add_argument('--output', default='./output360', help="")
    parser.add_argument(
        '--save_image',
        action='store_true',
        default=False,  #False
        help='')
    parser.add_argument('--outlier_filter',
                        action='store_true',
                        default=False,
                        help='')
    ...
    #                      "09":9,
    #                      "10":10}
    # output_dict = { "02":'../output/kitti360/epoch10/02',
    #                 "03":'../output/kitti360/epoch10/03',
    #                 "04":'../output/kitti360/epoch10/04',
    #                 "05":'../output/kitti360/epoch10/05',
    #                 "06":'../output/kitti360/epoch10/06',
    #                 "07":'../output/kitti360/epoch10/07',
    #                 "09":'../output/kitti360/epoch10/09',
    #                 "10":'../output/kitti360/epoch10/10'}

    # for seq in seq_list:
    #     args.test_sequence = test_sequence_dict[seq]
    #     args.output = output_dict[seq]
    #     print(f"args.test_sequence:type:type{type(args.test_sequence)} -- {args.test_sequence}")
