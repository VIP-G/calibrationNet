...
from models.utils import quat2mat, rotate_back, rotate_forward, tvector2mat, quaternion_from_matrix
...
class GeometricLoss(nn.Module):
    ...
    def forward(self, target_transl, target_rot, transl_err, rot_err):
        ...
        loss_rot = quaternion_distance(rot_err, target_rot,
                                       rot_err.device).mean()
...
class ProposedLoss(nn.Module):
    ...
    def forward(self, target_transl, target_rot, transl_err, rot_err):
        ...
        if self.rescale_trans != 0.:
            loss_transl = self.transl_loss(transl_err,
                                           target_transl).sum(1).mean() * 100
        loss_rot = 0.
...
class DistancePoints3D(nn.Module):
    ...
    def forward(self, point_clouds, target_transl, target_rot, transl_err,
                rot_err):
        """
...
class CombinedLoss(nn.Module):
    ...
    def forward(self, point_clouds, target_transl, target_rot, transl_err, rot_err):
        ...
        if self.rescale_trans != 0.:
            loss_transl = self.transl_loss(transl_err, target_transl).sum(1).mean()
        if  math.isnan(loss_transl):
            print('aaa',loss_transl)
            transl_err = transl_err.cpu()
            loss_transl = transl_err.detach().numpy()
        loss_rot = 0.
        if self.rescale_rot != 0.:
            loss_rot = quaternion_distance(rot_err, target_rot, rot_err.device).mean()
        if  math.isnan(loss_rot):
            rot_err = rot_err.cpu()
            loss_rot = transl_err.detach().numpy()
        pose_loss = self.rescale_trans * loss_transl + self.rescale_rot * loss_rot

        # start = time.time()
        point_clouds_distance_loss = torch.tensor([0.0]).to(transl_err.device)
        for i in range(len(point_clouds)):
            point_cloud_gt = point_clouds[i].to(transl_err.device)
            point_cloud_out = point_clouds[i].clone().to(point_cloud_gt.device)
            ...
            R_predicted = quat2mat(rot_err[i])
            T_predicted = tvector2mat(transl_err[i])
            RT_predicted = torch.mm(T_predicted, R_predicted).to(RT_target.device)

            RT_total = torch.mm(RT_target.inverse(), RT_predicted).to(point_cloud_gt.device)

            point_cloud_out = rotate_forward(point_cloud_out, RT_total)

            error = (point_cloud_out - point_cloud_gt).norm(dim=0)
            error.clamp(100.)
            point_clouds_distance_loss += error.mean()

 
        # end = time.time()
        # print("3D Distance Time: ", end-start)
        total_loss = 0.5 * pose_loss + 0.5* (point_clouds_distance_loss/target_transl.shape[0]) 

        self.loss['total_loss'] = total_loss
        self.loss['transl_loss'] = loss_transl
        self.loss['rot_loss'] = loss_rot
        self.loss['point_clouds_distance_loss'] = point_clouds_distance_loss / target_transl.shape[0]
