...
class DatasetLidarCameraKittiOdometry(Dataset):
    ...
    def __init__(self, args, split='val', device='cpu', suf='.png'):
        ...
        self.split = split
        self.val_sequence = args.val_sequence
        ...
        print('当前误差',self.max_r, self.max_t)
        #获取 内参、外参真值和all_files
        for seq in self.sequence_list:
            odom = odometry(self.root_dir, seq)
            calib = odom.calib
            self.K[seq] = calib.K_cam2  # 3x3 内参
            #激光雷达------->相机的变换矩阵
            self.GTs_T_cam02_velo[seq] = calib.T_cam2_velo
            image_list = os.listdir(
                os.path.join(self.root_dir, 'sequences', seq, 'image_2'))
            image_list.sort()
            # 获取指定目录下的图像文件列表并按名称排序
            print('ZGHZseq',seq, seq in self.val_sequence)
            for image_name in image_list:
                if not os.path.exists(
                        os.path.join(self.root_dir, 'sequences', seq,
                                     'velodyne',
                ...
                # if seq == self.val_sequence:
                #     if split.startswith('val') or split == 'test':
                #         self.all_files.append(
                #             os.path.join(seq,
                #                          image_name.split('.')[0]))
                # elif (not seq == self.val_sequence) and split == 'train':
                #     self.all_files.append(
                #         os.path.join(seq,
                #                      image_name.split('.')[0]))  # 00/image
        ...
        if split == 'val' or split == 'test':
            val_RT_file = os.path.join(
                self.root_dir, 'sequences',
                f'val_RT_left_seq{self.val_sequence}_{self.max_r:.2f}_{self.max_t:.2f}.csv'
            )
    ...
    def __getitem__(self, idx):
        ...
        # 段代码的用途是在点云处理任务中，将原始点云数据从相机坐标系转换到激光雷达坐标系，以便进行后续的配准、特征提取等操作
        RT_gt = self.GTs_T_cam02_velo[seq].astype(np.float32)
        ...
        depth_img, uv, reflect_img = lidar_project_depth(pc_rotated, 
                                                         calib, 
                                                         real_shape, 
                                                         reflectance)
        ...
        # image.save(f'./outputzgh/{rgb_name}.png')
        # ==================================================
        # --------------->>> 填充大小一致 <<<-----------------
        # ==================================================
        # odom 数据集图像大小不一致
